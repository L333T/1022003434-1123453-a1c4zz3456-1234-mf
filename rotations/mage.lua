...
function mage.interrupt(player, unit)
    ...
    if not gui.is_on("counterspell") or not learned(counterspell) then
        return false
    end
    ...
    return cast_unit(live("counterspell", counterspell), unit, "Counterspell") == true
end
...
function mage.tick(player, target, ctx)
    ...
    if mage.interrupt(player, target) then
        return true
    end
