-- ============================================================================
-- Master Farmer - Grindbot  ::  _MF NETWORK LOADER
...
-- main is the repository default. Each launch asks GitHub which commit that
-- branch points at and downloads that snapshot. A push is live on the next
-- load; this file does not store a commit, so it does not go stale.
local BRANCH = "main"
...
local LOADER_KEY = (NS._loader and NS._loader.key) or "MF_LOADER"
...
local resolve_gen = 0
local asked_at = 0
local next_resolve = 0
local RESOLVE_RETRY = 5.0
local RESOLVE_WAIT = 20.0
...
local function resolve_branch(done)
    ...
    resolve_gen = resolve_gen + 1
    local gen = resolve_gen
    asked_at = izi.now()
    ...
    local ok = pcall(function()
        core.http_get(REF_URL, headers, function(http_code, _, body)
            ...
            if sha then
                ...
                resolve_state = "idle"
                next_resolve = izi.now() + RESOLVE_RETRY
                core.log_warning(string.format(
                    "%s could not resolve %s@%s (http %s); retrying. No commit is pinned.",
                    TAG, REPO, BRANCH, tostring(http_code)))
            end
        end)
    end)
    ...
    if not ok then
        resolve_state = "idle"
        next_resolve = izi.now() + RESOLVE_RETRY
        core.log_warning(TAG .. " branch lookup could not be sent; retrying.")
    end
end
...
local function on_update()
    ...
    if resolve_state == "asking" then
        if (izi.now() - asked_at) > RESOLVE_WAIT then
            resolve_gen = resolve_gen + 1
            resolve_state = "idle"
            next_resolve = izi.now() + RESOLVE_RETRY
            core.log_warning(TAG .. " branch lookup timed out; retrying.")
        end
        return
    end
