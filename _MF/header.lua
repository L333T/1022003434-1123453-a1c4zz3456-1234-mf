...
-- Loader version: 1.0c    (this is the LOADER's version, not the bot's - the
--                          bot's version is whatever the manifest reports)
-- ============================================================================
-- Install this folder on its own as the plugin. It downloads the bot from
-- GitHub on every load, resolving the current main commit first, so a push to
-- the repo is what the next launch runs. Nothing in this folder is pinned to
-- a bot version.
--
...
local LOADER_KEY     = "MF_LOADER"
local LOADER_VERSION = "1.0c"

local plugin = {}
plugin["name"]      = "Master Farmer (_MF)"
plugin["short_tag"] = "MF"
...
local function refuse(msg)
    if msg then core.log_error("[MF] " .. msg) end
    plugin["load"] = false
    return plugin
end
